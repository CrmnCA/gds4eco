...
`{margin} Data If you want to read more about the data sources behind this dataset, head to the [Datasets](../data/datasets) section`
...
\`\`\`\`{admonition} Point geometries from columns :class: dropdown
...
`` {admonition} Challenge Make three plots of `sat`, plotting one single band in each ``
...
`{admonition} Challenge Give [Task III](https://darribas.org/gds_course/content/bC/diy_C.html#task-iii-the-gender-gap-on-the-streets) in this block of the GDS course a go`
